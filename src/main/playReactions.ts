// A thumbs-up in the Play feed, offered to the memory profile.
//
...
// as a list of things they like. A thumbs-down lives in `play_items.reaction`
// and in the two prompts, which is where it is actually useful.

import * as database from './database'
import { MEMORY_FIELDS } from '../shared/memoryCatalog'
import { PLAY_MEMORY_FIELD_KEYS } from './playPlanner'
import type { MemoryCandidate, PlayItem } from '../shared/types'
 ...
 * channel attached.
 */
function reactionValue(item: PlayItem, suggested: string | null): string {
  const creator = item.creator?.trim()
  if (creator) return creator.slice(0, 60)
  return (suggested ?? item.title).trim().slice(0, 60)
}

export function memoryCandidatesForReaction(item: PlayItem, fieldKey: string | null): MemoryCandidate[] {
  if (!fieldKey) return []
  ...
  if (!PLAY_MEMORY_FIELD_KEYS.includes(fieldKey)) return []
  ...
  return [
    {
      ...
      rationale: 'Thumbs-up in the Play feed',
      sources: [
        {
          type: 'play-reaction',
          reference: `play:${item.id}`,
 ...
 */
function fieldKeyForItem(item: PlayItem): string | null {
  return database.getPlayItemMemoryFieldKey(item.id)
}
 ...
 */
export function applyReactionToMemory(item: PlayItem, fieldKeyOverride?: string | null): void {
  if (database.hasPlayReactionBeenWritten(item.id)) return
  ...
  try {
    ...
    database.markPlayReactionWritten(item.id)
  } catch {
