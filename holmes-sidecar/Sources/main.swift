...
Usage: holmes-sidecar healthkit [--type <dataType>] [--days <N>] [--json]
       holmes-sidecar photokit
       holmes-sidecar corelocation

Subcommands:
  healthkit    Read Apple Health samples. --type 'all' or one of:
               steps, heartRate, restingHeartRate, heartRateVariability,
               bloodPressureSystolic, bloodPressureDiastolic, oxygenSaturation,
               bodyMass, bodyMassIndex, bodyFatPercentage, sleepAnalysis, workout,
               activeEnergy, exerciseMinutes, standHours, flightsClimbed,
               distanceWalkingRunning, dietaryEnergy, caffeine, water,
               bloodGlucose, electrocardiogram.
               --days lookback window (default 7, max 3650).
  photokit     Emit metadata for the most recent 1000 photo assets (dates, location, media type).
  corelocation Emit a one-shot CoreLocation fix (10s timeout).
...
func hkUnit(forCode code: String) -> HKUnit {
    switch code {
...
func queryCumulative(_ store: HKHealthStore, descriptor: Descriptor, days: Int) async throws -> [Observation] {
    ...
    let unit = hkUnit(forCode: descriptor.code)
        ...
        let query = HKSampleQuery(
            ...
            if descriptor.code == "HKCategoryTypeIdentifierSleepAnalysis" {
                ...
                let observations = grouped.compactMap { (value, samples) -> Observation? in
                    ...
                    let label: String
                    switch value {
                    case 0: label = "Sleep In Bed"
                    case 1: label = "Sleep Asleep"
                    case 2: label = "Sleep Asleep Unspecified"
                    case 3: label = "Sleep Core"
                    case 4: label = "Sleep Deep"
                    case 5: label = "Sleep REM"
                    case 6: label = "Sleep Awake"
...
func runHealthKit(_ argv: [String]) async {
    guard let parsed = parseArgs(argv) else { exit(2) }
...
func runPhotoKit() async {
    ...
    case .authorized, .limited:
        break
    default:
        writeJSON(StatusResponse(status: "needs_permission"))
        exit(0)
    }
    ...
    let metas: [PhotoAssetMeta] = assets.map { asset in
        let kind: String
        ...
        return PhotoAssetMeta(
            occurredAt: asset.creationDate,
            assetKind: kind,
            lat: coord?.latitude,
            lng: coord?.longitude,
            locationName: nil
...
final class LocationDelegate: NSObject, CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        guard !fired, let loc = locations.last else { return }
        fired = true
...
final class LocationFetcher {
    ...
    func fetch(timeoutSeconds: TimeInterval) async -> LocationFix? {
        await withCheckedContinuation { (continuation: CheckedContinuation<LocationFix?, Never>) in
            self.continuation = continuation
            self.manager.delegate = self.delegate
            self.manager.desiredAccuracy = kCLLocationAccuracyBest

            self.delegate.onFix = { [weak self] fix in
                self?.complete(with: fix)
            }
            self.delegate.onError = { [weak self] _ in
                self?.complete(with: nil)
            }

            let workItem = DispatchWorkItem { [weak self] in
                guard let self = self, !self.delegate.fired else { return }
                self.delegate.fired = true
    ...
    private func complete(with fix: LocationFix?) {
...
func runCoreLocation() async {
    ...
    let fix = await fetcher.fetch(timeoutSeconds: 10.0)

    if let fix = fix {
        writeJSON(fix)
    } else {
        writeJSON(StatusResponse(status: "timeout"))
    }
    exit(0)
}

@main
struct HolmesSidecar {
    static func main() async {
        let argv = CommandLine.arguments
        guard argv.count >= 2 else {
            FileHandle.standardError.write(Data("""
Usage: holmes-sidecar <subcommand> [options]
Subcommands: healthkit, photokit, corelocation
Run 'holmes-sidecar --help' for details.
...
""".utf8))
        ...
        switch subcommand {
        case "healthkit":
            await runHealthKit(rest)
        case "photokit":
            await runPhotoKit()
        case "corelocation":
            await runCoreLocation()
        case "--help", "-h":
            _ = parseArgs(["--help"])
            exit(0)
        default:
